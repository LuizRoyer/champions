  ...
  findProducersWithIntervals = async (movies: TableMovies[]) => {
    ...
    movies.forEach((movie) => {
      ...
      const previousMovie = movies.find(
        (m) => m.producers === producer && m.year < year,
      );
      if (previousMovie) {
        ...
        producersWithIntervals.push({
          producer,
          interval,
          previousWin: previousMovie.year,
          followingWin: year,
        });
      }
    });

    const minIntervals = producersWithIntervals.filter(
      (interval) =>
        interval.interval ===
        Math.min(...producersWithIntervals.map((i) => i.interval)),
    );
    const maxIntervals = producersWithIntervals.filter(
      (interval) =>
        interval.interval ===
        Math.max(...producersWithIntervals.map((i) => i.interval)),
    );
