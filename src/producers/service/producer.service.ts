...
import { ReadCSV } from "src/producers/service/read.csv.service";
...
export class ProducerService {
    ...
    constructor(private readonly csv: ReadCSV) {  }

    async getClassification():Promise<Classification> {
        
       const champions = await this.csv.readCSV('src/public/movielist.csv');      
        return this.findProducersWithIntervals(champions);
    }

    async getProductions():Promise<Production[]> {
        
        return  this.csv.readCSV('src/public/movielist.csv');      
